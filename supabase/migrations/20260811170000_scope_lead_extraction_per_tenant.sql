-- =====================================================================
-- Escopo por tenant do motor de chips (trg_lead_needs_extraction)
--
-- PROBLEMA
-- A bateria de regex desta função é o VOCABULÁRIO CLÍNICO DA CLÍNICA ÓR
-- (cetamina, ketamina, EMT, EMDR, psicoterapia, ...). Até aqui ela rodava em
-- TODA mensagem inbound de TODAS as clínicas do banco, escrevendo
-- `procedimento_interesse`, `demonstrou_interesse`, `tentou_pagamento`,
-- `qualificacao='desqualificado'`, `is_b2b` e mais 8 chaves em leads de tenants
-- que não têm nada a ver com psiquiatria.
--
-- DECISÃO
-- Chips continuam vindo de regex, porém POR TENANT. Nenhum default global:
-- clínica sem a flag ligada não recebe extração alguma.
-- Ver docs/roadmap/RASCUNHO_SEPARACAO_FLUXO_TENANT.md — decisões D1 e D3.
--
-- ESCOPO DESTA MIGRAÇÃO (Fase 0 — contenção)
-- Apenas o GATE. A bateria de regex continua embutida na função, ainda com o
-- vocabulário da ÓR. A Fase 3 move os padrões para uma tabela por clínica
-- (`clinic_extraction_rules`) e transforma esta função em avaliador genérico.
-- A função não carrega nenhum UUID de clínica: o tenant é nomeado apenas no
-- seed de dados no fim do arquivo.
--
-- EFEITO COLATERAL DESEJADO
-- Leads de outras clínicas param de receber `needs_ai_review = true` por este
-- caminho, esvaziando a fila-lixo do classifier (Sanapta, MKart, MK Espanha,
-- FEBRACIS órfã) que hoje é descartada a cada tick como `clinic_not_allowlisted`.
-- O enfileiramento legítimo do classifier V2 NÃO passa por aqui — vem de
-- `tg_enqueue_classifier`, que é gateado por `pipeline_tenant_classifiers.enabled`.
-- =====================================================================
...
BEGIN
  ...
  -- ── GATE POR TENANT ────────────────────────────────────────────────────
  -- Sem flag ligada, nenhuma extração. Ausência = desligado (sem default global).
  IF NOT COALESCE(
       (SELECT (c.settings->>'lead_extraction_enabled')::boolean
          FROM public.clinics c
         WHERE c.id = v_lead.clinic_id),
       false)
  THEN
    RETURN NEW;
  END IF;
...
COMMENT ON FUNCTION public.trg_lead_needs_extraction() IS
  'Motor de chips por regex. Gateado por clinics.settings.lead_extraction_enabled '
  '(sem default global). O vocabulário embutido é da Clínica ÓR — Fase 3 move os '
  'padrões para tabela por clínica. Ver docs/roadmap/RASCUNHO_SEPARACAO_FLUXO_TENANT.md';

-- ── Seed: hoje somente a Clínica ÓR tem extração ligada ────────────────────
UPDATE public.clinics
   SET settings = COALESCE(settings, '{}'::jsonb)
                || jsonb_build_object('lead_extraction_enabled', true)
 WHERE id = 'cf038458-457d-4c1a-9ac4-c88c3c8353a1';
