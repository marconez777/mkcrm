// PDF ingestion via base64 upload. Uses unpdf (pure JS, works in Deno) to extract text.
...
Deno.serve(async (req) => {
  ...
  try {
    const { agent_id = null, title, file_base64 } = await req.json();
    ...
    const { data: doc, error: docErr } = await supabase
      ...
      .insert({
        ...
        content: cleaned,
        source: "pdf",
        metadata: { pages: pdf.numPages, size: bin.byteLength },
      })
      .select("id")
      .single();
