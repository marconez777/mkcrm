// Fetches a URL, strips HTML to plain text, and ingests it as a knowledge document.
...
Deno.serve(async (req) => {
  ...
  try {
    const { agent_id = null, url, title: customTitle } = await req.json();
    ...
    if (ctype.includes("html")) {
      ...
      return json({ error: `unsupported content-type ${ctype}` }, 400);
    }

    if (text.length < 50) return json({ error: "extracted text too short" }, 400);

    // Title from <title> if available
    let title = customTitle as string | undefined;
    if (!title) {
      const m = /<title[^>]*>([^<]+)<\/title>/i.exec(text) ?? null;
      title = m?.[1]?.trim() || url;
    }
