// Shared template variable renderer for messages (templates, sequences, quick replies, automations).
// Supports legacy variables and {{campo.<field_key>[:modifier]}} for custom fields.
...
function parseDate(v: any): Date | null {
  ...
  if (!s) return null;
  // Accept "YYYY-MM-DD" as local-noon to avoid TZ drift
...
function formatCustom(value: any, fieldType: string, modifier: string | null, tz: string): string {
  ...
  // Infer date/datetime even when the custom field isn't registered with a type:
  // a date modifier or an ISO-looking value is enough evidence.
