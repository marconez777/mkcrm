...
> ⚠️ **Esta skill foi reescrita em 11-12/08/2026.** A versão anterior descrevia uma
...
>
> **O que existe de verdade é `npm run docs:verify`** (§5) — criado em 12/08 e
> focado em drift entre código e registries, não em `code_refs`.
...
## 5. Finalização: verificar drift

```bash
npm run docs:verify        # falha (exit 1) se houver divergência
npm run docs:verify:warn   # só reporta
```

`scripts/docs-verify.mjs` compara o código do pipeline com os registries em
`docs/pipeline/runtime/_registry/`. Existe porque três garantias de segurança
**falsas** — "strict no-move", "tags sempre MERGE", "conflito humano 24h" —
sobreviveram meses na documentação sem quebrar nada. Prosa não falha build;
linha ausente de registry, sim.

Ele **não** valida `code_refs`. Para isso, e para o resto, confira à mão:
