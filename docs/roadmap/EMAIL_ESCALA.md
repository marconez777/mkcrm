...
| **G-39** 🔴 | `check_clinic_bounce_health` (trigger) | a pausa automática por bounce > 5% / reclamação > 0,3% **só muda o status da campanha** (`running`/`sending`/`scheduled` → `paused`); **não toca em `email_queue`**, e o worker não olha status de campanha — os e-mails continuam saindo. E desde 21/08 a campanha vai para `sent` ao ser enfileirada: `sent` não está na lista, então campanha em pleno envio é invisível ao trigger | **não existe freio automático que pare um envio ruim em andamento.** Com 16% de bounce acumulado no MCD, isso é o item mais perigoso do módulo. Corrigir: o trigger deve pausar a fila (`pending → paused` da clínica) e incluir `sent` com `sent_at` recente; achado ao escrever `docs/tenants/mcd/email/ENTREGABILIDADE.md` |
