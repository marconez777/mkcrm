...
| **G-33** 🔴 | `send-email-batch` fase 2 | `const { data: claims }` sem checar `error`. Se `claim_send_dedup_batch` falhar, `claims` vem nulo, o mapa fica vazio e `claimedMap.get(email) === false` dá falso para todos — **o lote inteiro segue como se o dedup tivesse sido reservado, sem nenhuma linha inserida** | é a única proteção contra envio duplicado. Com ela furada, uma reexecução do mesmo lote entrega duas vezes ao mesmo destinatário |
| **G-34** 🟠 | `send-email-batch` fases 3 e 4 | idem para `claim_email_quota_bulk` e `claim_domain_warmup_bulk`: sem checar `error`, `granted` vira 0 | um erro transitório de banco **reagenda a campanha inteira para 12:00 UTC de amanhã** com motivo "cota atingida", sem cota nenhuma ter sido atingida |
| **G-35** 🟡 | `send-email-batch` caminho de falha do Resend | libera dedup e warm-up **um por um** (N idas ao banco, justo quando o lote inteiro falhou) e **não devolve a cota** — `release_email_quota_bulk` existe e não é chamada | vaga de cota consumida sem envio. Inofensivo no MCD (50M/dia), relevante na ÓR (100k) |
| **G-36** 🔴 | teto de 8s vale para **toda** chamada PostgREST | `service_role` não tem `statement_timeout` próprio, mas quem abre a conexão é o papel **`authenticator`**, que tem `statement_timeout=8s`. `SET ROLE` não reseta a configuração de sessão. Logo, **RPC chamada por edge function também é capada em 8s** | derruba a premissa do F2.1: enfileirar 146k numa transação via RPC **nunca** vai caber. Medido em 21/08 — a campanha "teste" foi para `failed` com `canceling statement due to statement timeout` |
