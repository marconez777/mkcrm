...
| Campo | Tipo | Enum / formato | Quem escreve | Quem lê |
|---|---|---|---|---|
| `is_b2b` | bool | — | `trg_lead_needs_extraction` (regex F2) + extractor-tick (confirmação) | field-rules-tick (prio 200) |
| `tipo_contato` | enum string | `paciente` · `terceiro_familiar` · `b2b` · `fornecedor` · `imprensa` · `outro` | `trg_lead_needs_extraction` (pré-marca `b2b`) + extractor-tick | field-rules-tick (prio 200) |
| `contato_eh_terceiro` | bool | — | extractor-tick | UI Lead Drawer (alerta) |
| `responsavel` | string | nome do paciente real quando `contato_eh_terceiro=true` | extractor-tick | UI Lead Drawer |
| `interesse_inicial` | enum string | `psiquiatria` · `psicologia` · `emt` · `cetamina` · `avaliacao` · `indefinido` | extractor-tick | métricas (canal de entrada) |
| `servicos_ativos` | string[] | subset de `["emt","cetamina","psiquiatria","psicologia"]` | extractor-tick | field-rules-tick (Em tratamento) |
| `risco_clinico` | bool | — | `trg_lead_needs_extraction` (regex F2) + extractor-tick | `trg_lead_risk_handler` (trava lead, **DESABILITADO até F6**) |
| `risco_clinico_detectado_em` | timestamptz | quando o trigger disparou pela 1ª vez | `trg_lead_needs_extraction` | UI Lead Drawer (badge) |
