...
- O agente é criado com `enabled=false` + `draft_mode=true`. Ele **não responde leads reais** até o usuário ativar manualmente em `/ai/agents?agent=<id>` (toggle "Ativo" dentro da aba "Agentes" do AiHub — `src/pages/Agents.tsx`). Test Lab e KB Assistant funcionam mesmo com o agente desativado.
