...
### 2.4. `supabase/functions/watch-stale-leads/index.ts` (ou o Cron respectivo de Inatividade)
- **O que fazer:** Alterar as queries de inatividade para a nova cascata:
  - `where horas_sem_resposta >= 24 and stage in ('Qualificação', 'Consulta agendada', 'Consulta finalizada', 'Tratamento agendado')` -> **Move to `Sem resposta`**.
  - `where horas_sem_resposta >= 72 and stage = 'Sem resposta'` -> **Move to `Nutrição inativa`**.
  - `where dias_sem_resposta >= 60 and stage = 'Paciente antigo'` -> **Move to `Nutrição inativa`**.
- **Por que mitiga erro:** O modelo atual pode estar configurado para apenas colocar flag ou mover direto para inativa.
