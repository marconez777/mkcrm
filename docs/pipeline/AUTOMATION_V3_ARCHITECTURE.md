...
## 🕵️ Auditores IA de Retaguarda (Dupla Checagem)

Para garantir que a automação primária (Maestro) não estrague o funil, existem auditores silenciosos que revisam as ações sem interferir no tempo real da conversa.

1. **`pipeline-post-move-verifier`**: Um agente ultrarrápido (`gpt-5-nano`) que roda logo *após* a IA mover um lead de coluna. Ele revisa os 5 eventos mais recentes para validar se a movimentação fez sentido. Se ele achar que a IA primária errou feio (confiança >= 80%), ele **não reverte** o move, mas taga o lead com `precisa_atencao_humana` e `post_move_warning` para o operador humano inspecionar a falha.
2. **`pipeline-position-auditor`**: Um cron diário (roda de madrugada) que funciona como o bedel do funil. Ele procura leads que estão **parados há mais de 7 dias** na mesma coluna (e não estão agendados). O auditor invoca um modelo revisor (`gpt-5-mini`). Se a IA perceber que o lead foi abandonado na coluna errada, ela taga com `auditor_sugere_<coluna>` e cria uma tarefa manual (`Revisar posição: auditor sugere <X>`) para a secretária olhar. Ele **jamais** move cards por conta própria.

---

## 🔁 Follow-ups e Comunicação Autônoma

1. **`agent-followups-tick`**: Um cron que roda a cada 5 minutos medindo o SLA. Se o médico/clínica configurou que a coluna X precisa de follow-up em 48h, e o lead está nela há mais de 48h sem resposta, esse agente é ativado. Se houver uma mensagem configurada, ele adiciona na fila de disparo automático. Se não houver, ele cria uma anotação interna invisível sugerindo que o humano faça o follow-up.
