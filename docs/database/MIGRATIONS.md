...
| 2026-05-28 | Revoke `whatsapp_instances`/`form_integrations`/`ai_agents` secrets para `anon`, trigger idempotente `trg_email_queue_campaign_counters`, índices `email_segment_contacts`/`leads`, trigger `trg_email_logs_suppress_on_bounce`, novo `trigger_type='pipeline_enter'` em `message_sequences` |
